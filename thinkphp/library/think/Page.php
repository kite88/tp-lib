...
class Page{
    ...
    public function __construct($totalRows, $listRows=20, $parameter = array()) {
       // C('VAR_PAGE') && $this->p = C('VAR_PAGE'); //设置分页参数名称
        ...
        $p = $page ? $page : input('param.p');
        $maxSize = input('param.maxSize');
        $listRows = $maxSize ? $maxSize : $listRows;
...
//                    $link_page .= '<span class="current">' . $page . '</span>';
    ...
    {
        $page = array(
            'pageIndex' => (int)$this->nowPage,
            'totalPages' => (int)$this->totalPages,
            'totalRecores' => (int)$this->totalRows,
            'maxSize' => (int)$this->listRows,
        );
        return $page;
    }
}
